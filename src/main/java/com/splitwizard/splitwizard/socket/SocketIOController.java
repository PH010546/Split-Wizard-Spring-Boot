...
public class SocketIOController {
    ...
    public DataListener<Message> onSendMessage = new DataListener<>() {
        @Override
        public void onData(SocketIOClient client, Message message, AckRequest acknowledge) throws Exception {
             ...
             */
            List<String> targetUserIds = message.getTargetUserIds();

            for (String targetUserId : targetUserIds) {
                log.info("memberId [" + message.getSenderId() + "] send message to memberId [" + targetUserId + "] and message is " + message.getMessage());
                socketServer.getBroadcastOperations().sendEvent(targetUserId, client, message);
            }
